@tool
extends  Resource
class_name Character_parametrs

@export var max_run_speed: float = 135.0

@export var max_sprint_speed: float = 180.0

@export var walk_accel: float = 337.5

@export var run_accel: float = 337.5

@export var stop_decel: float = 225.0

@export var p_meter_starting_dpeed = 131.25

@export var max_p_meter = 1.867

@export var walk_decel: float = 562.5

@export var run_decel: float = 1125.0

@export var walk_accel_slippery: float = 112.5

@export var run_accel_slippery: float = 337.5

@export var stop_decel_slippery: float = 18.125

@export var spin_jump_speed_iner: float = 8.671875

@export var gravity_with_jump_held: float = 675.0

@export var gravity_without_jump_held: float = 1350.0

@export var base_jump_speed: float = 300.0

@export var jump_speed_incr: float = 9.375

@export var base_spin_jump_speed: float = 277.5
